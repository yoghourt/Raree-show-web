...
async function runJudge(
  ...
  if (!apiKey?.trim()) {
    throw new Error("OPENROUTER_API_KEY is required for OpenRouter semantic evaluation")
  }

  const modelId = process.env.EVAL_OPENROUTER_MODEL?.trim() ?? "openai/gpt-oss-120b:free"
...
export function createOpenRouterJudgeAdapter(): SemanticJudgeAdapter {
  return {
